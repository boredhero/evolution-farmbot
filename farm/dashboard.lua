...
local D={};D.__index=D
local statusColors={ready='5',growing='d',unknown='8',stale='8',debt='e',busy='4',unsupported='2',excluded='7'}
local symbols={ready='R',growing='g',unknown='?',stale='?',debt='!',busy='*',unsupported='?',excluded='x'}
local function count(t) local n=0;for _ in pairs(t or {}) do n=n+1 end;return n end
local function short(name) return (name or '?'):gsub('^mysticalagriculture:','MA:'):gsub('^minecraft:','MC:') end
...
function D:view(name)
  if not self.views[name] then
    ...
    self.views[name]={role=hasMap and 'stats' or 'map',zoom=1,cache=true,page=1}
  end
...
function D:render(m,v,w,h)
  ...
  f:fill(1,1,w,2,' ','0','b')
  f:write(3,1,'FARMBOT / '..(role=='map' and 'TACTICAL MAP' or 'OPERATIONS'), '0','b')
  ...
  f:write(math.max(3,w-28),1,health,m.active and '5' or '4','b')
  f:write(3,2,'Display refresh 2s | scan '..(m.scanAge<0 and 'pending' or m.scanAge..'s old')..' | maturity = last observed','8','b')
  if w<80 or h<36 then
    f:write(2,4,'Expand this monitor for the war-room layout.','4')
    f:write(2,6,'Plots '..count(m.plots)..' | workers '..count(m.workers))
    f:write(2,8,'Lifetime harvests '..m.metrics.harvests)
    return f,hits
  end
  if role=='map' then
    local layer=pickLayer(m,v)
    local x=3
    x=button(f,hits,x,3,'[FLOOR -]','layer',-1)
    x=button(f,hits,x,3,'[FLOOR +]','layer',1)
    x=button(f,hits,x,3,'[AUTO]','auto')
    x=button(f,hits,x,3,'[ZOOM -]','zoom',-1)
    x=button(f,hits,x,3,'[ZOOM +]','zoom',1)
    x=button(f,hits,x,3,v.cache==false and '[CACHE OFF]' or '[CACHE ON]','cache')
    if x+23<w then
      x=button(f,hits,x,3,'[N]','pan','n');x=button(f,hits,x,3,'[S]','pan','s')
      x=button(f,hits,x,3,'[W]','pan','w');button(f,hits,x,3,'[E]','pan','e')
    end
    ...
    f:write(3,5,('CROP Y=%d  |  X %d..%d  Z %d..%d  |  NORTH ^  |  %dx zoom'):format(layer,minX,minX+span-1,minZ,minZ+span-1,v.zoom or 1),'3')
    ...
    for dy=0,mh-1 do for dx=0,mw-1 do
      ...
      elseif m.world:blocked(p) then ch='#';fg='7'
    ...
    for k,p in pairs(m.plots) do if p.job.pos.y==layer then
      ...
      if px then
        ...
        if not occupied[cell] or priorities[status]>=occupied[cell] then
          f:write(px,py,symbols[status],statusColors[status]);occupied[cell]=priorities[status]
    ...
    local legendY=h-5
    f:write(3,legendY,'R ready seen','5');f:write(18,legendY,'g growing seen','d');f:write(35,legendY,'! replant gap','e')
    f:write(51,legendY,'? unknown/stale','8');f:write(71,legendY,'* in flight','4')
    f:write(87,legendY,': cached','7');f:write(100,legendY,'. active route','3')
    local fleet={}
    for i,id in ipairs(ids) do
      local worker=m.workers[id]
    ...
    f:write(3,h-3,'Layers: '..table.concat((function() local a={};for _,r in ipairs(levels(m)) do a[#a+1]=r.y..' ('..r.n..')' end;return a end)(),' / '),'8')
    if v.selected and m.plots[v.selected] then
      local p=m.plots[v.selected];local history=m.history[v.selected]
      f:write(3,h-2,(v.selected..'  '..p.job.name..'  ['..D.classify(p,history,m)..']'):sub(1,w-4),'0')
      ...
      f:write(3,h-1,('Observation: %s%s | Click another crop to inspect'):format(seen,age and ' | age '..age..'/'..tostring(p.job.age or '?') or ''),'8')
    else f:write(3,h-2,'Touch a crop for details. Cached paths are stored history, not a promise they remain clear.','8') end
  else
    ...
    for i,c in ipairs(cards) do local xx=2+(i-1)*(third+1)
      f:box(xx,5,third,5,c[1]);f:write(xx+2,7,c[2]:sub(1,third-4),'5');f:write(xx+2,8,c[3]:sub(1,third-4),'8')
    end
    f:write(3,11,('Sessions %d | Surveys %d | Errors/path failures %d | Cache %d routes / %d hits'):format(metrics.sessions,metrics.scans,metrics.errors,count(m.cache),m.cacheHits),'3')
    local ids={};for id in pairs(m.workers) do ids[#ids+1]=id end;table.sort(ids)
    local y=13
    ...
    if #ids==0 then f:write(3,y,'Waiting for paired workers...','8');y=y+1 end
    y=y+1
    local tableBottom=math.max(y+3,h-11);local perPage=math.max(1,tableBottom-y-2)
    ...
    local nameWidth=w-64
    f:write(3,y,pad('CROP',nameWidth)..string.format(' %5s %5s %6s %6s %7s %8s','LIVE','GAPS','READY*','GROW*','UNSEEN*','LIFE HV'),'8');y=y+1
    for i=(v.page-1)*perPage+1,math.min(v.page*perPage,#sorted) do
      local row=sorted[i];local lifetime=metrics.byCrop[row.name]
      f:write(3,y,pad(short(row.name),nameWidth)..string.format(' %5d %5d %6d %6d %7d %8d',row.live,row.gaps,row.ready,row.growing,row.unknown,lifetime and lifetime.harvests or 0),row.gaps>0 and 'e' or '0');y=y+1
    end
    f:write(3,h-9,'RECENT ACTIVITY / harvest actions, not item quantities','3')
    local events=metrics.events
    for i=0,4 do local event=events[#events-i];if event then
      f:write(3,h-8+i,(math.floor(m.now-event.time)..'s  '..event.outcome..'  '..short(event.name)..(event.detail and ' | '..event.detail or '')):sub(1,w-5),event.outcome=='deferred' and 'e' or '8')
    end end
    f:write(3,h-2,'* Last observed maturity; unseen/stale is NOT assumed ripe. Lifetime counters survive restarts.','8')
    f:write(3,h-1,'Lifetime telemetry since '..tostring(math.floor(metrics.since))..' UTC epoch; uptime counts controller runtime, including pauses.','7')
  end
  if m.scanError then f:write(3,4,('SCANNER: '..m.scanError):sub(1,w-5),'e') end
  return f,hits
end
function D:draw(mon,name,m)
  if mon.getTextScale()~=0.5 then mon.setTextScale(0.5);self.frames[name]=nil end
  local w,h=mon.getSize();local frame,hits=self:render(m,self:view(name),w,h)
