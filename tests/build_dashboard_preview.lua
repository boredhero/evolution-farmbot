...
local palette={'f0f0f0','f2b233','e57fd8','99b2f2','dede6c','7fcc19','f2b2cc','4c4c4c',
...
for _,role in ipairs({'map','stats'}) do
  local frame=D.new():render(fixture(),{role=role,zoom=1,cache=true},164,81)
  local file=assert(io.open('dashboard-'..role..'-preview.html','w'))
  file:write('<!doctype html><meta charset="utf-8"><title>FarmBot '..role..' preview - simulated data</title><style>body{margin:0;background:#111;color:#aaa;font:14px monospace}header{padding:10px}svg{display:block}text{font:12px monospace;white-space:pre}</style><header>FARMBOT '..role:upper()..' / SIMULATED DEMO DATA / actual dashboard renderer</header><svg xmlns="http://www.w3.org/2000/svg" width="1312" height="972" viewBox="0 0 1312 972">')
  for y=1,frame.h do
