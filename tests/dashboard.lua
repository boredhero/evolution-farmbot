...
local function cellAt(frame,ch)
  for y=1,frame.h do local text,fg,bg=frame:row(y)
    local at=text:find(ch,1,true)
    while at do if bg:sub(at,at)~='f' then return y,at,bg:sub(at,at) end;at=text:find(ch,at+1,true) end
  end
end
...
assert(contains(d:render(m,{role='map',zoom=1},121,81),'player detector unavailable'))
m.playerError=nil
print('PASS player overlay marks the viewer, other players and off-floor position without a detector requirement')
